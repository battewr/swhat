...
# Command file content for AI agents (Claude Code, Roo)
SPECIFY_COMMAND_CONTENT = """\
---
...
The text the user typed after `/swhat.specify` in the triggering message **is** the feature description. Assume you always have it available in this conversation even if `$ARGUMENTS` appears literally below. Do not ask the user to repeat it unless they provided an empty command.
...
def initialize_project() -> bool:
    ...
    if claude_command_file.exists():
        claude_command_file.write_text(SPECIFY_COMMAND_CONTENT)
    ...
    if roo_command_file.exists():
        roo_command_file.write_text(SPECIFY_COMMAND_CONTENT)
        click.echo("  Updated .roo/commands/swhat.specify.md")
    else:
        roo_command_file.write_text(SPECIFY_COMMAND_CONTENT)
        click.echo("  Created .roo/commands/swhat.specify.md")

    click.echo("Initialization complete! You can now use /swhat.specify in Claude Code or Roo.")
    return True
